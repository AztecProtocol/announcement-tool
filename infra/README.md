...
| `DATABASE_SSL_ROOT_CERT` | path/contents of the CA bundle for the certificate Caddy obtained | Required whenever `DATABASE_SSL_MODE=verify-full` — without it, verification falls back to the system trust store, which is a false sense of security, not a lesser one. |
