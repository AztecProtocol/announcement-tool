 ...
 *
 * The gate runs at enqueue: countFanoutTargets does not create a
 * delivery_ledger row for a disabled channel, so nothing downstream has to
 * cope with one.
 *
 * Two backstops sit behind that. They are backstops, not the mechanism — an
 * earlier version of this comment described the unguarded failure as if it
 * were live, and a reviewer duly "found" it:
 ...
 *
 * This must never rewrite history. It governs composing, previewing and
 ...
 *
 * Absent or blank means all channels, which preserves the behaviour every
 * existing deployment has today — this variable is opt-in, so an operator who
 * has not heard of it keeps their five channels.
 *
